...
from models_graph import DICHarmonicGraphEncoder, GuidanceAdapter
...
def main():
    ...
    graph_model = DICHarmonicGraphEncoder()
    ...
    if 'h' in datasets:
        train_hook = 'data/DIC/hook' + '_mel'*use_melody + '_train.pkl'
        print('loading hook: ', train_hook)
        val_hook = 'data/DIC/hook' + '_mel'*use_melody + '_test.pkl'
    ...
    if 'j' in datasets:
        train_gjt = 'data/DIC/gjt' + '_mel'*use_melody + '_train.pkl'
        print('loading gjt: ', train_gjt)
        val_gjt = 'data/DIC/gjt' + '_mel'*use_melody + '_test.pkl'
    ...
    if 'n' in datasets:
        train_nottingham = 'data/DIC/nott' + '_mel'*use_melody + '_train.pkl'
        print('loading nottingham:', train_nottingham)
        val_nottingham = 'data/DIC/nott' + '_mel'*use_melody + '_test.pkl'
    ...
    if 'w' in datasets:
        train_wikifonia = 'data/DIC/wiki' + '_mel'*use_melody + '_train.pkl'
        print('loading wikifonia: ', train_wikifonia)
        val_wikifonia = 'data/DIC/wiki' + '_mel'*use_melody + '_test.pkl'
    ...
    # save results
    results_path = os.path.join( 'results', version, 'dic_adapter' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.csv' )
    os.makedirs('results', exist_ok=True)
    os.makedirs(f'results/{version}/', exist_ok=True)

    save_dir = f'saved_models/{version}/dic_adapter/'
    os.makedirs('saved_models/', exist_ok=True)
    os.makedirs(f'saved_models/{version}/', exist_ok=True)
    os.makedirs(f'saved_models/{version}/dic_adapter/', exist_ok=True)
