...
from data_utils import AdapterDataset, adapter_collate_fn
from torch.utils.data import DataLoader, ConcatDataset
from models_graph import SimpleHarmonicGraphEncoder, GuidanceAdapter
from models_BiLSTM import TokenHarmonyBiLSTM
...
import os
import pickle
from train_utils import train_adapter_loop
import argparse
import sys
...
def main():
    ...
    graph_model = SimpleHarmonicGraphEncoder()
    ...
    if 'h' in datasets:
        train_hook = 'data/simple/hook' + '_mel'*use_melody + '_train.pkl'
        print('loading hook: ', train_hook)
        val_hook = 'data/simple/hook' + '_mel'*use_melody + '_test.pkl'
        ...
        train_dataset_hook = AdapterDataset(train_d_hook, tokenizer, include_melody=use_melody)
        val_dataset_hook = AdapterDataset(val_d_hook, tokenizer, include_melody=use_melody)
        concat_train.append(train_dataset_hook)
        concat_val.append(val_dataset_hook)
    
    if 'j' in datasets:
        train_gjt = 'data/simple/gjt' + '_mel'*use_melody + '_train.pkl'
        print('loading gjt: ', train_gjt)
        val_gjt = 'data/simple/gjt' + '_mel'*use_melody + '_test.pkl'
        ...
        train_dataset_gjt = AdapterDataset(train_d_gjt, tokenizer, include_melody=use_melody)
        val_dataset_gjt = AdapterDataset(val_d_gjt, tokenizer, include_melody=use_melody)
        concat_train.append(train_dataset_gjt)
        concat_val.append(val_dataset_gjt)
    
    if 'n' in datasets:
        train_nottingham = 'data/simple/nott' + '_mel'*use_melody + '_train.pkl'
        print('loading nottingham:', train_nottingham)
        val_nottingham = 'data/simple/nott' + '_mel'*use_melody + '_test.pkl'
        ...
        train_dataset_nottingham = AdapterDataset(train_d_nottingham, tokenizer, include_melody=use_melody)
        val_dataset_nottingham = AdapterDataset(val_d_nottingham, tokenizer, include_melody=use_melody)
        concat_train.append(train_dataset_nottingham)
        concat_val.append(val_dataset_nottingham)

    if 'w' in datasets:
        train_wikifonia = 'data/simple/wiki' + '_mel'*use_melody + '_train.pkl'
        print('loading wikifonia: ', train_wikifonia)
        val_wikifonia = 'data/simple/wiki' + '_mel'*use_melody + '_test.pkl'
        ...
        train_dataset_wikifonia = AdapterDataset(train_d_wikifonia, tokenizer, include_melody=use_melody)
        val_dataset_wikifonia = AdapterDataset(val_d_wikifonia, tokenizer, include_melody=use_melody)
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=adapter_collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, collate_fn=adapter_collate_fn)

    logits_loss_fn = CrossEntropyLoss(ignore_index=-100)

    # optimizer = AdamW(transformer_model.film_parameters(), lr=lr)
    optimizer = AdamW(list(transformer_model.parameters()) + list(graph_model.parameters()) + list(bilstm_model.parameters()) + list(adapter_model.parameters()), lr=lr)

    # save results
    results_path = os.path.join( 'results', version, 'simple_adapter' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.csv' )
    os.makedirs('results', exist_ok=True)
    os.makedirs(f'results/{version}/', exist_ok=True)

    save_dir = f'saved_models/{version}/simple_adapter/'
    os.makedirs('saved_models/', exist_ok=True)
    os.makedirs(f'saved_models/{version}/', exist_ok=True)
    os.makedirs(f'saved_models/{version}/simple_adapter/', exist_ok=True)
    transformer_path = save_dir + f'transformer_model' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.pt'
    graph_model_path = save_dir + f'graph_model' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.pt'
    bilstm_model_path = save_dir + f'bilstm_model' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.pt'
    adapter_model_path = save_dir + f'adapter_model' + '_mel'*use_melody + '_contra'*use_contra + '_' + datasets + '.pt'
